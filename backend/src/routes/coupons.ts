...
router.post('/', async (req: Request, res: Response) => {
    try {
        ...
        const existing = await queryOne('SELECT id FROM coupons WHERE code = ?', [upperCode]);
        ...
        const coupon = await queryOne('SELECT * FROM coupons WHERE id = ?', [result.lastInsertRowid]);
        res.json({ coupon });
    } catch (error: any) {
        res.status(500).json({ error: error.message });
    }
...
router.put('/:id', async (req: Request, res: Response) => {
    try {
        ...
        const upperCode = code.trim().toUpperCase();
        const dup = await queryOne('SELECT id FROM coupons WHERE code = ? AND id != ?', [upperCode, id]);
...
router.patch('/:id/toggle', async (req: Request, res: Response) => {
    try {
        await execute('UPDATE coupons SET is_active = NOT is_active WHERE id = ?', [req.params.id]);
        const coupon = await queryOne('SELECT * FROM coupons WHERE id = ?', [req.params.id]);
        res.json({ coupon });
    } catch (error: any) {
        res.status(500).json({ error: error.message });
    }
